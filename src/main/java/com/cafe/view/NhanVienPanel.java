...
public class NhanVienPanel extends BaseManagementPanel {
    ...
    private void row(JPanel p, GridBagConstraints g, int y, String l1, JTextField f1, String l2, JTextField f2) {
        ...
        g.gridwidth = 1;
        g.gridx = 0;
        p.add(makeLabel(l1), g);
        g.gridx = 1;
        p.add(f1, g);
        g.gridx = 2;
        p.add(makeLabel(l2), g);
        g.gridx = 3;
        p.add(f2, g);
    }
    ...
    private void doSave() {
        try {
            ...
            try {
                ...
                showError(this, "Ngày sinh không hợp lệ");
                return;
            }
            ...
            if (!isValidSalary(l)) {
                showError(this, "Lương không hợp lệ");
                return;
            }
            nv.setLuong(new BigDecimal(l));
    ...
    private boolean isValidSalary(String input) {
        if (input == null || input.trim().isEmpty()) {
            return false;
        }

        try {
            BigDecimal salary = new BigDecimal(input.trim());
    ...
    private void doUpdate() {
        try {
            ...
            if (ma.isEmpty()) {
                showError(this, "Chọn hoặc nhập Mã NV!");
                return;
            }
            NhanVien nv = service.getById(ma);
            if (nv == null) {
                showError(this, "Nhân viên không tồn tại!");
                return;
            }
    ...
    private void doDelete() {
        String ma = txtMa.getText().trim();
        if (ma.isEmpty()) {
            showError(this, "Chọn nhân viên cần xóa!");
            return;
        }
        int c = JOptionPane.showConfirmDialog(this, "Xác nhận xóa nhân viên " + ma + "?", "Xác nhận",
